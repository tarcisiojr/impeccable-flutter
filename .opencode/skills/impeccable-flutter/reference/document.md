...
Sintetize em DESIGN.md "starter" com defaults sensatos baseados nas respostas, e marque cada section com `<!-- TODO: validate after first screens are built -->`. Lembre o user de re-rodar `/impeccable-flutter document` depois que tiver código.
...
Após escrever, **rode o loader de novo** (`node .opencode/skills/impeccable-flutter/scripts/load-context.mjs`) para a sessão pegar o DESIGN.md fresco. Comandos subsequentes vão usar.
...
Hand off para `/impeccable-flutter extract` se a documentação revelou padrões repetidos prontos para virar componentes.
